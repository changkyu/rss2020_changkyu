...
class CellPhysicsBase:
    ...
    def simulate(self, action, cnorm=lambda x: x, traj_names=None, traj_ref=None):

        time_start = time.time()
        self.n_sims = self.n_sims + 1

        names_target = action['targets']
        pos_act  = action[self.name_actor]['pos']
        yaw_act  = action[self.name_actor]['yaw']
        velocity = action[self.name_actor]['velocity']
        duration = action[self.name_actor]['duration']

        # place objects
        for name_target in names_target:
            pos_obj  = action[name_target]['pos']
            yaw_obj  = action[name_target]['yaw']
            self.sim.setObjectPose(name_target, pos_obj[0], pos_obj[1], yaw_obj)

        vec = np.array(velocity)/np.linalg.norm(np.array(velocity))
        self.sim.setObjectPose( self.name_actor, 
                                pos_act[0]-vec[0]*self.dims_actor[0], \
                                pos_act[1]-vec[1]*self.dims_actor[1], yaw_act)

        if self.is_vis: # fill Colors
            for name_target in names_target:
                cellinfo = self.sim.getCellBodyProperties(name_target)
                cnorm = matplotlib.colors.Normalize(vmin=self.mass_minmax[0],
                                                    vmax=self.mass_minmax[1])
                for info in cellinfo:
                    info['color'] = matplotlib.cm.coolwarm(cnorm(info['mass']))
                self.sim.setCellBodyProperties(name_target,cellinfo)

        if len(velocity)==2:
            velocity = [velocity[0],velocity[1],0]

        ret = self.sim.applyConstantVelocity(self.name_actor,velocity,duration,
                                             traj_names=traj_names,traj_ref=traj_ref)

        if self.is_dbg and 'pos_res' in action[names_target[0]]:
            xs={}
            ys={} 
            yaws={}
            for name_target in names_target:
                xs[name_target],ys[name_target],yaws[name_target]\
                 = self.sim.getObjectPose(name_target)
            ...
            for i in range(100):
                for name_target in names_target:
                    ...
                    self.sim.setObjectPose( name_target, x_res,y_res,yaw_res )
                self.sim.stepSimulation()

                for name_target in names_target:
                    self.sim.setObjectPose( name_target, xs[name_target],ys[name_target],yaws[name_target] )
                self.sim.stepSimulation()
