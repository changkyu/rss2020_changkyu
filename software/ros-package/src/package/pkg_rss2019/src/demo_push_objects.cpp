...
static bool use_marker=true;

char WaitUserConfirm( const string str_message="[Enter]", const string str_done="[OK]", 
                      const bool need_ret=false,          const string color=COLOR_RED )
{
    char ret;    
    if( use_pause || need_ret )
    {
...
int main(int argc, char* argv[])
{
    ...
    nh.getParam(prefix_param + "use_marker",use_marker);
    ROS_INFO_STREAM("use_robot: " << use_robot);
    ROS_INFO_STREAM("use_pause: " << use_pause);
    ROS_INFO_STREAM("use_marker: " << use_marker);
    if( use_robot )
    {
    ...
    double marker_size, marker_z;
    nh.getParam(prefix_param + "marker_size",marker_size);    
    nh.getParam(prefix_param + "marker_z",marker_z);    
    ...
    while(true)
    {
        ...
        for( int i=0; i<markers_push_0.size(); i++ )
        {
            if( markers_push_0[i].id==id_push )
            {
                ...
                if( marker_z >= 0.02 )
                    pose_push_pre.position.z = marker_z + finger_length + 0.10;
                else
        ...
        {        
            ...
            id_push = -1;
            if( 'r'!=WaitUserConfirm("Enter \"r\" for repeat?","[OK]",true) )
            {
