...
#include "pkg_rss2019/plan_pushing_srv.h"
#include "pkg_rss2019/save_history_srv.h"
...
{
    //std::vector<double> command_joint_position = {0, 17 * M_PI/180, 0, -72 * M_PI/180, 0, 90 * M_PI/180, 15 * M_PI/180};
...
int main(int argc, char* argv[])
{
    ...
    ros::ServiceClient plan_pushing  = nh.serviceClient<pkg_rss2019::plan_pushing_srv >("/pkg_rss2019/plan_pushing",  true);
    ros::ServiceClient save_history  = nh.serviceClient<pkg_rss2019::save_history_srv >("/pkg_rss2019/save_history",  true);
    ...
    while(true)
    {
        ...
        bool success = (err_xy <= 0.03) && (abs(err_yaw) <= 15.0/180.0*M_PI);
        ...
        if( is_first )
        {
            ...
            //objinfo.request.name = object_name;
            //objinfo.request.image = msg_img;
            //objinfo.request.meter_per_pixel = meter_per_pixel;
            //add_object.call(objinfo);
            //ROS_INFO("[Planner] add the object to planner - done");
        }
        else
        ...
        pkg_rss2019::plan_pushing_srv plan;
        plan.request.name = object_name;
        plan.request.image = msg_img;
        plan.request.init.x = marker_0.pose.position.x + x_tmp;
        plan.request.init.y = marker_0.pose.position.y + y_tmp;
        plan.request.init.theta = yaw0;
        plan.request.goal.x = marker_1.pose.position.x + x_tmp;
        plan.request.goal.y = marker_1.pose.position.y + y_tmp;
        plan.request.goal.theta = yaw1;
        plan.request.meter_per_pixel = meter_per_pixel;
        plan.request.pred_step = 1;
        ...
        int p=0;
        {
            ...
            if( use_robot )
            {
                ...
                //double frac = moveit_node->plan_and_execute_via_waypoints(pose_push_pre);
                //if( frac < 0.99 )
                //{
                //    moveit_node->plan_and_execute(pose_push_pre);
                //}
                pose_push_pre.position.z += 0.10;
                moveit_node->plan_and_execute(pose_push_pre);
                ros::Duration(1).sleep();                
                ...
                time_push = (time_push_end-time_push_beg).toSec();
                //WaitUserConfirm();
            ...
            marker_0.pose.position.x = plan.response.path[p+1].x;
            marker_0.pose.position.y = plan.response.path[p+1].y;
            ...
            observation.request.duration = time_push > 1 ? time_push : 0;
        }
    }

    if(use_prop)
    {
        ROS_INFO("[Save] save properties");
        pkg_rss2019::save_history_srv save;
        save.request.name = object_name;
        save.request.fp_save = dp_save + "prop.pkl";
        save_history.call(save);
        ROS_INFO("[Save] save properties - done");
    }
